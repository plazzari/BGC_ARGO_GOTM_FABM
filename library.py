...
def get_float_dates(bgc_df,bgc_float_name):
    data=bgc_df.loc[bgc_df['BIO-FLOAT'] == bgc_float_name]
    lista_date=[]
    for yyyymmdd in bgc_df['start'].values:
        yyyy=int(str(yyyymmdd)[0:4])
        mm  =int(str(yyyymmdd)[4:6])
        dd  =int(str(yyyymmdd)[6:8])
        HH  =int(str(yyyymmdd)[9:11])
        MM  =int(str(yyyymmdd)[12:14])
        SS  =int(str(yyyymmdd)[15:17])
        current_date=datetime.datetime(yyyy, mm, dd, HH, MM, SS)
